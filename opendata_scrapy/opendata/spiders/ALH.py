...
class QuotesSpider(scrapy.Spider):
        ...
        def start_requests(self):
            sf = False
            url= 'http://transparencia.alhaurindelatorre.es/dataset'
        ...
        def parse(self,response):
                ...
                if not self.sf:
                    ...
                    for l in links:
                        link_complteted = 'http://transparencia.alhaurindelatorre.es' + l
                        self.links_next.append(link_complteted)

                    next_page = 'http://transparencia.alhaurindelatorre.es/dataset?page=' + str(self.j)
                    print next_page
                ...
                if self.sf:
                    ...
                    date_creation = response.xpath('//*[@class="table table-striped table-bordered table-condensed"]/tbody/tr[2]//td/text()').extract_first(default='not-found')  
                    date_creation = date_creation.replace("\n","")
                    date_creation = date_creation.strip()

                    item['date_creation'] = date_creation

                    date_actualization = response.xpath('//*[@class="table table-striped table-bordered table-condensed"]/tbody/tr[1]//td/text()').extract_first(default='not-found')  
