...
class QuotesSpider(scrapy.Spider):
        ...
        def start_requests(self):
            sf = False
            url= 'http://gobiernoabierto.valencia.es/es/data/'
        ...
        def parse(self,response):
                ...
                if not self.sf:
                    links = response.xpath('//*[@class="dataset__title"]//@href').extract() 
                    ...
                    #print numeration
                    next_page = 'http://gobiernoabierto.valencia.es/es/data/?offset=' + str(numeration) + '&limit=10'
                ...
                if self.sf:
                    ...
                    desc_metadata = response.xpath('//*[@id="content"]/div/div/div[3]//p//text()').extract()                    
                    
                    count = 0
                    for i in desc_metadata:
                        if "Sector" in i.encode('utf-8'):
                            item['categories'] = [desc_metadata[count+1],desc_metadata[count+2]]
                    ...
                    if len(desc_metadata)>0:
                        item['description'] = desc_metadata[0]
                                                        
                    item['date_extraction'] = datetime.strftime(datetime.now(), '%Y/%m/%d_%H:%M:%S')

                    item['formats'] = response.xpath('//*[@class="w-85"]//text()').extract()              
                    
                    item['title'] = response.xpath('//*[@class="group"]//h1/text()').extract_first(default="not-found")
